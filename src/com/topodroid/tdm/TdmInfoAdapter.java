  ...
  { 
    ...
    void setViewText( FixedInfo info, TdmInfoAdapter adapter )
    {
      ...
      if ( info != null ) { 
        ...
        tvWGS84.setText(  String.format(Locale.US, "%1$.8f %2$.8f (%3$.0f) %4$.0f", info.lat, info.lng, info.alt, info.asl ) );
        if ( info.cs == null || info.cs.length() == 0 ) {
          llCS.setVisibility( View.GONE );
        } else {
          llCS.setVisibility( View.VISIBLE );
          tvCS.setText( info.cs + ":" );
          tvCSdata.setText(  String.format(Locale.US, "%1$.0f %2$.0f %3$.0f", info.cs_lat, info.cs_lng, info.cs_alt ) );
        }
      }

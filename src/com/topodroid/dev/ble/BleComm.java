...
{
  private static final boolean BLE_LOG = false;
  ...
  public BleComm( TopoDroidApp app, boolean use_mtu, UUID service_uuid, UUID chrt_read_uuid, UUID chrt_write_uuid )
  {
    ...
    mQueue = new BleQueue();
    if ( BLE_LOG ) TDLog.v("BLE comm. use MTU " + use_mtu );
  }
  ...
  {
    if ( BLE_LOG ) TDLog.v("BLE comm: changed MTU");
    if ( mUseMtu ) {
  ...
  { 
    if ( BLE_LOG ) TDLog.v("BLE comm: readed remote RSSI");
    mQps.clearPending();
  }
  ...
  { 
    if ( BLE_LOG ) TDLog.v("BLE comm: readed desc");
    mQps.clearPending();
  }
  ...
  {
    if ( BLE_LOG ) TDLog.v("BLE comm: completed reliable write");
    mQps.clearPending();
  }
  ...
  {
    if ( BLE_LOG ) TDLog.v("BLE comm: disconnect GATT");
  ...
  { 
    if ( BLE_LOG ) TDLog.v("BLE comm: request MTU " + mtu );

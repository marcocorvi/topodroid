...
class BricDebug
{
  ...
  {
    Log.v("BRIC", "MeasPrim: " + bytes.length + " date " 
  ...
  {
    Log.v("BRIC", "MeasMeta: " + bytes.length + " Idx " 
  ...
  {
    Log.v("BRIC", "MeasErr: " + bytes.length + " Err1 " 
  ...
  {
    if ( bytes != null ) {
      Log.v("BRIC", msg + " " + BleUtils.bytesToString( bytes ) );
    }
  }

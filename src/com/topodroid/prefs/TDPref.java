                      ...
                      , MyColorPicker.IColorChanged
{
  ...
  public static TDPref[] makeSvgPrefs( Context ctx, TDPrefHelper hlp )
  {
    ...
    return new TDPref[ ] {
      makeLst( cat, key[0],  tit[0],  dsc[0],  N,  def[9], R.array.svgProgram, R.array.svgProgramValue, ctx, hlp ), // DISTOC_SVG_PROGRAM
      makeCbx( cat, key[1],  tit[1],  dsc[1],  T,  def[1],         ctx, hlp ),
      makeCbx( cat, key[2],  tit[2],  dsc[2],  E,  def[2],         ctx, hlp ),
      makeCbx( cat, key[3],  tit[3],  dsc[3],  E,  def[3],         ctx, hlp ),
      makeCbx( cat, key[4],  tit[4],  dsc[4],  N,  def[4],         ctx, hlp ),
      // makeCbx( cat, key[ ], tit[ ], dsc[ ], X, def[ ],         ctx, hlp ),
      ...
      makeEdt( cat, key[10], tit[10], dsc[1],  A,  def[10], FLOAT,  ctx, hlp ),

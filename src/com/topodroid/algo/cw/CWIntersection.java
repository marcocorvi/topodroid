...
public class CWIntersection
{
  ...
  CWTriangle mTriA;  // triangle A
  CWTriangle mTriB;  // triangle B
   ...
   * @param ta    traingle A
   * @param tb    traingle B
   * @param v     base-point of intersection
   * @param n     direction of interscetion
   */
  ...
  void reverse()
  {
    CWLinePoint v = mV1; mV1 = mV2; mV2 = v; // swap points V1 and V2
    mV1.mAlpha *= -1;
    mV2.mAlpha *= -1;
    mN.reverse();
    CWTriangle t = mTriA; mTriA = mTriB; mTriB = t; // swap triangles
    mType += 2;
  ...
  /** set the next intersection in the chained list
   * @param next   next intersection
   */
  void setNext( CWIntersection next ) { mNext = next; }

  /** @return the next intersection in the chained list
   */
  ...
  // /*
  //  * @param other  another intersection
  //  * @return 1 (direct follow) -1 (reverse follow) 0 (not follow)
  //  *   mSign[0] == other.mSign[3]  the triangles of this V1 and the other V2 coincide
  //  *   mSign[1] == other.mSign[4]  the sides of this V1 on the other V2 concide
  //  *   mSign[2] == other.mSign[5]  the opposite-triangle of this V1 and that of the other's V1 coincide
  //  *                .                                .
  ...
  //  *                '                                '
  //  */
  // int followSignature( CWIntersection other ) 
  // {
  //   if ( mSign[0] == other.mSign[3] && mSign[1] == other.mSign[4] && mSign[2] == other.mSign[5] ) return  1;
  //   if ( mSign[5] == other.mSign[3] && mSign[1] == other.mSign[4] && mSign[3] == other.mSign[5] ) return -1;
  //   return 0;
  // }
  
  /** @return true if can follow direct
   * @param other  another intersection
   */
  boolean followSignatureDirect( CWIntersection other )
  {
    return ( mSign[0] == other.mSign[3] && mSign[1] == other.mSign[4] && mSign[2] == other.mSign[5] );
  }
  
  /** @return true if can follow reverse
   * @param other  another intersection
   */
  boolean followSignatureInverse( CWIntersection other )
  {
    return ( mSign[5] == other.mSign[3] && mSign[1] == other.mSign[4] && mSign[3] == other.mSign[5] );
  }
  ...
  //     TDLog.v( "CW-Hull CW " + mCnt + " " + mType + ": " + mTriA.mCnt + "-" + mTriB.mCnt
  ...
  //   CWTriangle t1 = mV1.mSide.otherTriangle( mV1.mTri );
  //   CWTriangle t2 = mV2.mSide.otherTriangle( mV2.mTri );
  //   TDLog.v( "CW-Hull CW: I " + mCnt + " [" + mType + ": tri " + mTriA.mCnt + " " + mTriB.mCnt
  //                  + " ] " + t1.mCnt + "/" + mV1.mSide.mCnt + "/" + mV1.mTri.mCnt
  //                  + " -- " + mV2.mTri.mCnt + "/" + mV2.mSide.mCnt + "/" + t2.mCnt 
